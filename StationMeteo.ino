...
void mettreAJourMinMax(float temperature, float humidite) {
  ...
  if (temperature < tempMin) { tempMin = temperature; changement = true; }
  if (temperature > tempMax) { tempMax = temperature; changement = true; }
  if (humidite < humMin) { humMin = humidite; changement = true; }
  if (humidite > humMax) { humMax = humidite; changement = true; }
  ...
  switch(etatMenu) {
    ...
    case MENU_PRINCIPAL:
      ...
      if (btnUp == LOW && prevUp == HIGH) {
        dernierAppuiBouton = maintenant;
        selectionMenu--;
        if (selectionMenu < 0) selectionMenu = NOMBRE_OPTIONS_MENU - 1;
        afficherMenuPrincipal();
      }
      ...
      if (btnDown == LOW && prevDown == HIGH) {
        dernierAppuiBouton = maintenant;
        selectionMenu++;
        if (selectionMenu >= NOMBRE_OPTIONS_MENU) selectionMenu = 0;
        afficherMenuPrincipal();
      }
